    ...
    inquirer.prompt([
        ...
        products.getItem("item_id",response.id).then((desiredItem)=>{
            if(!desiredItem[0]||!desiredItem[0].item_id){
                ...
                var id = desiredItem[0].item_id
                var name = desiredItem[0].product_name
                var price = desiredItem[0].price
                var receipt = [{ID:id,Name:name,"Unit Price":"$"+price,Quantity:response.amount,Subtotal:"$"+response.amount*price}]
